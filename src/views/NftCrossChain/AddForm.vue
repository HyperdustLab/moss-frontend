<template>
  <div>
    <div class="w-200 mx-auto mt-45">
      <el-button class="w-80 ml-72%" style="font-size: 20px" @click="nftCrossChainList.show()">See transaction history
    ...
    <el-card class="w-250 mx-auto h-150 mt-10">
      ...
      <el-form ref="ruleFormRef" label-position="top" :model="formData" :rules="rules" label-width="200px" class="demo-ruleForm w-200 ml-9% mt-10%" status-icon>
        <el-form-item label="Step1 Source Blockchain" prop="sourceBlockchainId">
          <el-input
              v-model="formData.sourceSmartContractAddress"
              readonly
              style="height: 80px;font-size: 20px"
              placeholder="Source SmartContract Address"
          ...
          >
            ...
            <template #append>
              <el-button @click="openSelectTargetSmartContractAddress">
                Select
              </el-button>
            </template>
          </el-input>
        ...
        <el-form-item class="mt-50" label="Step2 Source NFT Token Id" prop="sourceTokenId">
          <el-input
              v-model="formData.sourceTokenId"
              style="max-width: 800px;height: 80px;font-size: 20px"
              readonly
              placeholder="Source NFT Token Id"
              class="input-with-select"
          >
            <template #append>
              <el-button @click="openSelectTargetNFTToken">
                Select
              </el-button>
            </template>
          </el-input>
...
function submitForm() {
  ruleFormRef.value.validate(async valid => {
    if (valid) {
      ...
      try {
        await contract.safeTransferFrom(user.username, NFT_Receiving_Address, formData.sourceTokenId)
